...
@rem Build release
msbuild ..\src\TimeStampClient\TimeStampClient.csproj /p:Configuration=Release /target:Restore || goto :error
msbuild ..\src\TimeStampClient\TimeStampClient.csproj /p:Configuration=Release /target:Clean || goto :error
msbuild ..\src\TimeStampClient\TimeStampClient.csproj /p:Configuration=Release /target:Build || goto :error

@rem Copy packages to output directory
copy ..\src\TimeStampClient\bin\Release\*.nupkg %outputdir% || goto :error
copy ..\src\TimeStampClient\bin\Release\*.snupkg %outputdir% || goto :error
