...
@rem Build release
msbuild ..\src\TimeStampClientGui.Windows\TimeStampClientGui.Windows.csproj /p:Configuration=Release /target:Restore || goto :error
msbuild ..\src\TimeStampClientGui.Windows\TimeStampClientGui.Windows.csproj /p:Configuration=Release /target:Clean || goto :error
msbuild ..\src\TimeStampClientGui.Windows\TimeStampClientGui.Windows.csproj /p:Configuration=Release /target:Build || goto :error

@rem Copy binaries to the output directory
set APPDIR=TimeStampClientGui.Windows
rmdir /S /Q %APPDIR%
mkdir %APPDIR% || goto :error
xcopy ..\src\TimeStampClientGui.Windows\bin\Release\ %APPDIR% /E  || goto :error
del %APPDIR%\*.pdb
del %APPDIR%\*.xml
...
copy license\Eto.Platform.Wpf.txt %LICDIR% || goto :error
